...
class FakeMT5:
    ...
    def order_send(self, request):
        ...
        self.positions.append(SimpleNamespace(ticket=100 + self.calls, symbol=request["symbol"], comment=request.get("comment", "")))
...
def bound_profile(tmp_path, mt5):
    ...
    return {"path": str(terminal), "login_id": 1001, "server": "Broker-Demo"}


def test_mutation_gateway_rejects_without_bound_identity(tmp_path):
    mt5 = FakeMT5()
    store = SQLiteStore(str(tmp_path / "state.db"))
    result = send_mutation_idempotent(request(), "close:demo:70", mt5_module=mt5, mutation_store=store)
    store.close()
    assert result["status"] == "REJECTED"
    assert "MUTATION_PROFILE_REQUIRED" in result["error"]
    assert mt5.calls == 0
...
def test_existing_order_is_reconciled_without_send(tmp_path):
    ...
    store = SQLiteStore(str(tmp_path / "state.db"))
    first = send_order_idempotent(request(), "scheduled:demo:42", mt5_module=mt5, profile_config=profile)
    second = send_order_idempotent(request(), "scheduled:demo:42", mt5_module=mt5, profile_config=profile)
    store.close()
...
def test_mutation_gateway_rejects_account_switch_before_repeat_send(tmp_path):
    mt5 = FakeMT5()
    profile = bound_profile(tmp_path, mt5)
    store = SQLiteStore(str(tmp_path / "state.db"))
    first = send_mutation_idempotent(request(), "close:demo:71", mt5_module=mt5, mutation_store=store, profile_config=profile)
    store.close()
    mt5.login = 2002
    second_store = SQLiteStore(str(tmp_path / "state.db"))
    second = send_mutation_idempotent(request(), "close:demo:71", mt5_module=mt5, mutation_store=second_store, profile_config=profile)
    second_store.close()
    assert first["status"] == "DONE"
    assert second["status"] == "REJECTED"
    assert "ACCOUNT_MISMATCH" in second["error"]
    assert mt5.calls == 1
...
def test_ambiguous_exception_after_broker_acceptance_is_reconciled(tmp_path):
    ...
    profile = bound_profile(tmp_path, mt5)
    result = send_order_idempotent(request(), "copy:demo:123", mt5_module=mt5, profile_config=profile)
...
def test_invalid_fill_is_the_only_retryable_send_error(tmp_path):
    ...
    profile = bound_profile(tmp_path, mt5)
    result = send_order_idempotent(request(), "scheduled:demo:42", mt5_module=mt5, profile_config=profile)
...
def test_mutation_unknown_is_not_retried_blindly(tmp_path):
    ...
    profile = bound_profile(tmp_path, mt5)
    store = SQLiteStore(str(tmp_path / "state.db"))
    result = send_mutation_idempotent(request(), "close:demo:77", mt5_module=mt5, mutation_store=store, profile_config=profile)
    store.close()
    assert result["status"] == "UNKNOWN"
    assert mt5.calls == 1
...
def test_mutation_explicit_rejection_is_not_retried_without_invalid_fill(tmp_path):
    ...
    store = SQLiteStore(str(tmp_path / "state.db"))
    result = send_mutation_idempotent(request(), "modify:demo:77", mt5_module=mt5, mutation_store=store, profile_config=profile)
    store.close()
    assert result["status"] == "REJECTED"
...
def test_mutation_unknown_survives_restart_and_blocks_second_send(tmp_path):
    ...
    first = send_mutation_idempotent(
        request(), "close:demo:900", mt5_module=mt5, mutation_store=first_store, profile_config=profile
    )
    first_store.close()
    second_store = SQLiteStore(str(db_path))
    second = send_mutation_idempotent(
        request(), "close:demo:900", mt5_module=mt5, mutation_store=second_store, profile_config=profile
    )

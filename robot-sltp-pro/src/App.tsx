...
const UI_COPY = {
  vi: {
    ...
    patternHint: 'ngày giao dịch trước · lookback 4 nến H4 mới → cũ · base = cây #4: Sw đảo chiều base, Bt giữ chiều base.', reverseHint: 'Reverse cuối: H3 T2/T5 + T6 theo chu kỳ tháng · H7 T2/T3/T6 · H9 T5/T6 · H12 trừ T4 · H14 tất cả.', evidenceClickHint: 'Mẹo: Click trực tiếp vào dòng Pattern trong từng ô để xem chart 4 nến và dữ liệu OHLC làm bằng chứng.', evidenceTitle: 'Bằng chứng 4 nến H4', evidenceHint: 'Chart trái → phải = cũ → mới · OHLC lấy trực tiếp từ 4 nến lookback.', refresh: 'Làm mới MT5', refreshing: 'Đang tính…', patternEmpty: 'Chưa có dữ liệu Pattern5. Nhấn Làm mới MT5.', patternLoading: 'Đang đọc H4/D1 broker-time, phân nhóm Sw/Bt và tính Reverse Signal…'
  },
  en: {
...
function Pattern5Panel({ data, loading, mt5Connected, onRefresh, ui }: { data: Pattern5Payload | null; loading: boolean; mt5Connected: boolean; onRefresh: () => void; ui: UiCopy }) {
  return <div className="feature-layout single pattern5-layout"><Panel title="PATTERN5 ENGINE · H4 BROKER-TIME" icon={<ActivityIcon size={20} />} className="pattern5-panel"><div className="pattern5-toolbar"><div className="pattern5-rule-stack"><div><b>Lookback:</b> {ui.patternHint}</div><div><b>Reverse:</b> {ui.reverseHint}</div></div><span className={`pattern5-status ${mt5Connected ? 'connected' : 'offline'}`}><span className="dot" /> MT5 {mt5Connected ? ui.connected : ui.offline}</span><button className="primary-button pattern5-refresh" onClick={onRefresh} disabled={loading}><Radio size={17} /> {loading ? ui.refreshing : ui.refresh}</button></div><div className="pattern5-click-hint"><Eye size={16} /> {ui.evidenceClickHint}</div><div className="pattern5-content">{!data && !loading && <div className="pattern5-empty">{ui.patternEmpty}</div>}{loading && <div className="pattern5-empty">{ui.patternLoading}</div>}{data?.tables.map((table) => <Pattern5TableView key={table.base} table={table} blocks={data.blocks} ui={ui} />)}</div></Panel></div>;
}

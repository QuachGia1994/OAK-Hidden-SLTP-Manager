...
import type { Activity, NavKey, Pattern5Payload, PendingTask, Position, Profile, RuntimeHealth } from './types';
...
const UI_COPY = {
  vi: {
    ...
    patternHint: 'ngày giao dịch trước · 4 nến H4 mới → cũ · signal H9/H14 đảo.', refresh: 'Làm mới MT5', refreshing: 'Đang tính…', patternEmpty: 'Chưa có dữ liệu Pattern5. Nhấn Làm mới MT5.', patternLoading: 'Đang đọc H4/D1 broker-time và phân loại Pattern1–5…'
  },
  en: {
    ...
    patternHint: 'previous trading day · 4 H4 candles newest → oldest · H9/H14 signals reversed.', refresh: 'Refresh MT5', refreshing: 'Calculating…', patternEmpty: 'No Pattern5 data yet. Press Refresh MT5.', patternLoading: 'Reading broker-time H4/D1 and classifying Pattern1–5…'
  }
} as const;
  ...
  return (
    <div className="app-shell">
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-status" aria-label={ui.profileMonitoring}><div className="status-line"><span className={`dot ${mt5Connected ? 'green' : 'red'}`} /> MT5: <b>{mt5Connected ? ui.connected : ui.offline}</b></div><div className="status-line"><span className={`dot ${runtimeHealth?.telegram.running ? 'green' : 'red'}`} /> Telegram: <b>{runtimeHealth?.telegram.running ? ui.connected : ui.offline}</b></div><div className="status-line"><span className={`dot ${runtimeHealth?.worker.running ? 'green' : 'red'}`} /> {ui.worker}: <b>{runtimeHealth?.worker.running ? ui.connected : ui.offline}</b></div><div className={`remote-status ${runtimeHealth?.remoteReady ? 'ready' : 'offline'}`}>{runtimeHealth?.remoteReady ? ui.remoteReady : ui.remoteOffline}</div><div className="status-version">{ui.version}: 0.1.0</div></div>
      </aside>
      ...
      <main className="main-content">
        ...
        <footer className="footer-bar"><div><Clock3 size={17} /><span>Ping</span><b>24ms</b></div><div><Radio size={17} /><span>Server</span><b>{selectedProfile.server}</b></div><div><Clock3 size={17} /><span>Uptime</span><b>02:15:36</b></div><div><ActivityIcon size={17} /><span>Last Update</span><b>14:32:18</b></div><div className="footer-copy">ROBOT SLTP Pro · <strong>{ui.footerTagline}</strong></div></footer>
      </main>
    </div>
  );
}
...
function Pattern5Panel({ data, loading, mt5Connected, onRefresh, ui }: { data: Pattern5Payload | null; loading: boolean; mt5Connected: boolean; onRefresh: () => void; ui: UiCopy }) {
  return <div className="feature-layout single pattern5-layout"><Panel title="PATTERN5 ENGINE · H4 BROKER-TIME" icon={<ActivityIcon size={20} />} className="pattern5-panel"><div className="pattern5-toolbar"><div><b>Lookback:</b> {ui.patternHint}</div><span className={`pattern5-status ${mt5Connected ? 'connected' : 'offline'}`}><span className="dot" /> MT5 {mt5Connected ? ui.connected : ui.offline}</span><button className="primary-button pattern5-refresh" onClick={onRefresh} disabled={loading}><Radio size={17} /> {loading ? ui.refreshing : ui.refresh}</button></div><div className="pattern5-content">{!data && !loading && <div className="pattern5-empty">{ui.patternEmpty}</div>}{loading && <div className="pattern5-empty">{ui.patternLoading}</div>}{data?.tables.map((table) => <Pattern5TableView key={table.base} table={table} blocks={data.blocks} />)}</div></Panel></div>;
}

function Pattern5TableView({ table, blocks }: { table: Pattern5Payload['tables'][number]; blocks: number[] }) {
  if (table.error) return <div className="pattern5-error"><b>{table.base}</b> · {table.error}</div>;
  return <div className="pattern5-card"><div className="pattern5-heading"><strong>{table.base}</strong><span>{table.symbol && table.symbol !== table.base ? `→ ${table.symbol}` : table.symbol}</span><em>Week {table.days?.[0]?.date.slice(0, 7)}</em></div><div className="pattern5-scroll"><table className="pattern5-table"><thead><tr><th>Block</th>{table.days?.map((day) => <th key={day.date}>{day.name}<small>{day.display}</small></th>)}</tr></thead><tbody>{blocks.map((block) => <tr key={block}><th>H{block}</th>{table.rows?.[String(block)]?.map((cell, index) => <td key={`${block}-${index}`} title={table.detail?.[String(block)]?.[index] || ''}>{typeof cell === 'string' || !cell ? <span className="pattern5-muted">—</span> : <><b className={cell.signal === 'BUY' ? 'buy' : 'sell'}>{cell.group}</b><small>{cell.signal}</small><em>Pat{cell.pattern}{cell.mirrored ? '*' : ''}</em></>}</td>)}</tr>)}</tbody></table></div></div>;
}

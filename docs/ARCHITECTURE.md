...
7. One worker process owns the passive H1 Telegram scanner through the existing OS-backed scanner lock. It resolves XAUUSD/EURUSD/AUDUSD/USDCAD/USDJPY suffix variants. XAU starts at broker H04 using H03→H02; EUR/AUD/CAD/JPY start at H03 using H02→H01. Later slots use the three latest closed H1 candles newest→oldest through H17. The scanner also resolves GBPUSD (including broker suffixes) as its H1 reference: scanner slots H03-H05/H06-H08/H09-H11/H12-H14/H15-H17 map to canonical Pattern5 blocks H3/H6/H9/H12/H15; the immediately previous closed GBPUSD H1 candle is the backward base, Sw/Sr reverse it and Bt follows it. Pattern5 group classification remains owned by `pattern5_engine.py`; H15 is independent like the other blocks. For each target symbol and broker day, the first H1 pattern match locks the daily classification: the newest pattern candle maps T→BUY and G→SELL, then that symbol H1 direction is compared with the GBPUSD H1 reference at the same scan slot. Same direction classifies the symbol/day as `SW`; opposite direction classifies it as `BT`. If GBPUSD reference data is not yet sufficient, the earliest pending match is retried rather than sending an unclassified alert. After the first match locks `SW/BT`, the scanner continues through H17 and sends Telegram for every later TGG/GTT match in chronological order. For those later alerts, the target pattern is only the trigger: the target H1 signal is derived from the current GBPUSD H1 reference using the locked daily classification (`SW` reverses GBPUSD, `BT` follows GBPUSD). Existing delivered rows are enriched with canonical signal/detail fields without Telegram replay. Whenever normalized state changes, and once when a scanner owner first observes unchanged state, `h1_signal_public_feed.py` publishes a separate versioned H1 snapshot to Upstash; publication failure is advisory and cannot roll back Telegram/state or affect order/runtime logic. The locked daily classification cannot change. User-visible alert numbering and the old even-number caution reminder are not used. It does not execute orders or alter Engine5 active-symbol scope.

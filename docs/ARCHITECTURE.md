...
| Cloud Telegram command/control | `dashboard/src/app/api/telegram/webhook/route.ts` + `telegram-cloud-domain.ts` / `telegram-cloud-store.ts` | Telegram → Upstash intents/audit + cTrader read-only status |

...
| --- | --- | --- |
| Engine 5 lookback/classification/Sw-Bt/base/reverse/final signal/H14 reference | `robot-sltp-pro/pattern5_engine.py` | Tauri, publisher, web feed |
...
3. `pattern5_engine.py` alone calculates classification, Sw/Bt, base signal, reverse, final signal and `h14Reference`.
...
8. `Pattern5Board.tsx` renders the supplied payload. In particular, H14 reference is formatted from backend-provided `h14Reference`; the browser does not reconstruct previous-trading-day logic.

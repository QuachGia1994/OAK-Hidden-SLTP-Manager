...
7. One worker process owns the passive H1 Telegram scanner through the existing OS-backed scanner lock. It resolves XAUUSD/EURUSD/AUDUSD/USDCAD/USDJPY suffix variants. XAU starts at broker H04 using H03→H02; EUR/AUD/CAD/JPY start at H03 using H02→H01. Later slots use the three latest closed H1 candles newest→oldest through H17. Each Telegram alert also derives a GBPUSD H1 reference signal: scanner slots H03-H05/H06-H08/H09-H11/H12-H14/H15-H17 map to canonical Pattern5 blocks H3/H6/H9/H12/H15; the immediately previous closed GBPUSD H1 candle is the base, Sw/Sr reverse it and Bt follows it. Pattern5 group classification remains owned by `pattern5_engine.py`; H15 is independent like the other blocks, while missing GBPUSD/H4 data produces an explicit N/A signal rather than a fabricated one. Delivery state is persisted per broker day/base symbol, and alert numbers 2/4/6/... add a caution reminder without a daily alert quota. It does not execute orders or alter Engine5 active-symbol scope.

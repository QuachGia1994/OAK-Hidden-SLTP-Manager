...
- H1 signal rule v5/state v11 replaces the pure-only cooldown with an H8+ non-overlapping allowTrade lookback for scanner Pattern 1 (`TGG`/`GTT`): prior Pattern 1 or Pattern 2 (`TTT`/`GGG`) blocks the slot; prior Pattern 3 (`GTG`/`TGT`) reverses the signal once before the independent calendar post-signal step; scanner Pattern 2 bypasses lookback. Web/mobile detail views expose the lookback reason, and older rule feeds are rejected instead of carrying stale trade state forward.

...
- H1 signal rule v9/state v15 extends the target-specific pair gates while preserving the H3/H4 early slots, H8+ fallback allowTrade, DST handling and Thursday/Friday post-signal pipeline. FX now scans SW3 at H5 and applies the GBPUSD H2→H1 pair gate at both H5 and H6; XAUUSD applies the XAUUSD H3→H2 pair gate at both H6 and H7. In every pair gate `TG`/`GT` is `BLOCK / NOT TRADE`, while `TT`/`GG` leaves the computed signal unchanged. XAUUSD H5 remains inactive and FX H7 has no pair gate. The retired pure-slot cooldown/following-slot block remains removed. Public schema stays 7 and older signal-rule feeds are rejected under rule v9.

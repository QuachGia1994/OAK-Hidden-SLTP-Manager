...
- H1 signal rule v7/state v13 adds two-candle early slots while preserving the H8+ allowTrade fallback logic. FX targets use GBPUSD `TG`/`GT`/`TT`/`GG` only at H3 and follow their own H2 base; XAUUSD uses its own two-candle pattern only at H4 and follows GBPUSD H3. Both early slots are always trade-eligible, keep the base signal, and only then apply the existing Thursday/Friday special post-signal inversion. H5 is inactive; the SW3 scanner remains H06-H16. Public schema stays 7, older rule feeds are rejected, and the route now exposes the resolved broker UTC offset while broker-time conversion is regression-tested as IC Markets New-York-close UTC+2/UTC+3 across exact US DST boundaries.

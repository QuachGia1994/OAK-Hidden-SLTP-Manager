...
- H1 signal rule v11/state v17 removes H5 as a signal slot for every target while preserving H3/H4 early slots, H6/H7 pair gates, H8+ fallback allowTrade, DST handling and Thursday/Friday post-signal ordering. H5 is not calculated, stored, historically reconstructed or published. FX pair-gating now applies only at H6 using GBPUSD H2→H1; XAUUSD pair-gating remains H6/H7 using XAUUSD H3→H2. EURUSD, AUDUSD and USDCAD continue to reverse their own H1 base signal; XAUUSD and USDJPY continue to keep their configured base. Public schema stays 7 and older signal-rule feeds are rejected under rule v11.

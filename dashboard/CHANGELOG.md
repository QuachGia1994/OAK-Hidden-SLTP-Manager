...
- H1 signal rule v8/state v14 adds target-specific pair gates while preserving the H3/H4 early slots, H8+ fallback allowTrade, and Thursday/Friday post-signal pipeline. FX H6 inspects GBPUSD H2→H1; XAUUSD H7 inspects XAUUSD H3→H2. In either gate `TG`/`GT` is `BLOCK / NOT TRADE`, while `TT`/`GG` leaves the computed signal unchanged. XAU H6 and FX H7 are not pair-gated. The retired pure-slot cooldown/following-slot block remains removed. Public schema stays 7 and older signal-rule feeds are rejected so persisted trade state cannot be relabeled under rule v8.

...
- Removed the `BT +1 · SW +2` helper line from the shared H1 `ENTRY TIME` row and PNG export, with the PNG entry row compacted to match the signal-row height. H1 timing and signal logic are unchanged.
...
- Upgraded the public NeoTech C5 helper to v1.09 with standalone local popup + chart `C5 LOOK`, account auto-bind across MT5 switches, direct EX5/checksums, and an embedded one-click Windows Setup that detects MT5 data folders and installs the verified binary. Telegram forwarding is optional; full controller provisioning is intentionally hidden under Advanced/Operator.
...
- Advanced local H1 to v88: all H3-H14 signal bases now use H(entry-2). XAUUSD/GBPAUD/GBPCAD/GBPJPY retain previous available broker-day GBPUSD/AUDUSD/USDCAD/USDJPY mappings, GBPUSD remains same-day own GBPUSD, H16 remains entry-only, and stale v87 H(entry-1) rows are rejected pending source-bar rebuild.
- Advanced local H1 to v87: XAUUSD remains the shared entry-time owner, while XAUUSD/GBPAUD/GBPCAD/GBPJPY derive H3-H14 BUY/SELL from previous available broker-day GBPUSD/AUDUSD/USDCAD/USDJPY H(entry-1). GBPUSD keeps same-day own H(entry-1); Monday resolves Friday; H16 stays entry-only. Local snapshots again include the three hidden base symbols and previous-day bars for live/history reconstruction.
- Advanced local H1 to v86: the matrix is now five rows (`XAUUSD`, `GBPUSD`, `GBPAUD`, `GBPCAD`, `GBPJPY`) and every row calculates all six H3/H6/H9/H12/H14/H16 blocks Monday-Friday. XAUUSD alone owns entry-time pattern detection; every row copies that entry hour and derives its own BUY/SELL from its own same-day H(entry-1) candle after the H1 candle closes. GBPUSD is independent for direction, EURUSD is retired from H1 only, H16 remains entry-only, and schema 18/state v56 plus broker execution independence are unchanged. Stale v85 pattern rows are rejected rather than relabeled.
- Advanced local H1 to v85: Monday excludes GBPAUD/GBPCAD/GBPJPY from every H1 block while XAUUSD keeps normal Monday calculation and GBPUSD/EURUSD retain H9/H12/H14/H16. Retained/public state drops stale Monday cross alerts without changing public schema 18/state v56. Web highlights XAUUSD H3 and H14 together whenever the XAUUSD H3 entry is H4; this is presentation-only and does not mutate H14 signal logic.
- Unified Web H1 Live + History into one `/engine` surface with a compact broker-date popover; `/history` now redirects to `/engine` and the closed calendar grid is not mounted in the normal DOM.
- Advanced local H1 to v84: GBPCAD and GBPJPY now calculate the same full weekday block set as GBPAUD (`H3/H6/H9/H12/H14/H16`) with their dedicated USDCAD/USDJPY scanners; all three GBP crosses continue using previous-broker-day GBPUSD at H(entry-2) for final BUY/SELL on H3-H14. H16 is restored across Web/native clients as pattern/evidence + entry-time only and carries no base direction, scheduled side or BUY/SELL signal. GBPUSD/EURUSD remain synced to XAUUSD on H9/H12/H14 only; H16 uses local entry calculation with no signal. Public schema 18/state v56 stay stable.
- Advanced local H1 to v83: GBPAUD, GBPCAD and GBPJPY again scan AUDUSD, USDCAD and USDJPY respectively for pattern/evidence and SW/BT entry timing, while all three final BUY/SELL signals use previous-broker-day GBPUSD at H(entry-2). XAUUSD remains XAUUSD scanner + GBPUSD H(entry-2); GBPUSD/EURUSD H9/H12/H14 remain synced to XAUUSD. Public schema 18/state v56 stay stable for an in-place 90-day History rebuild.
- Advanced local H1 to v82: GBPAUD, GBPCAD and GBPJPY now all scan GBPUSD for pattern/evidence and SW/BT entry timing, while final BUY/SELL returns to dedicated previous-broker-day H(entry-2) bases: AUDUSD, USDCAD and USDJPY respectively. XAUUSD remains on XAUUSD scanner + GBPUSD H(entry-2); GBPUSD/EURUSD H9/H12/H14 remain synced to XAUUSD. Public schema 18/state v56 stay stable for an in-place 90-day History rebuild.
- Advanced local H1 to v81: GBPAUD, GBPCAD and GBPJPY now scan AUDUSD, USDCAD and USDJPY respectively, so their pattern/evidence and SW/BT entry timing come from their own scanner source. All three then use previous-broker-day GBPUSD at H(entry-2) for final BUY/SELL, matching XAUUSD's base rule. GBPUSD/EURUSD H9/H12/H14 remain synced to XAUUSD. Public schema 18/state v56 remain stable for an in-place history rebuild.
...
- Advanced local H1 to v76. GBPUSD and EURUSD H9/H12/H14/H16 again copy XAUUSD's exact entry hour and already-final BUY/SELL for the same block, while retaining the existing GBPUSD-driven pattern/evidence window. Evidence on Web/native iOS/native Android identifies `XAUUSD Hxx` as FINAL SOURCE with `SYNC XAUUSD`. GBPJPY now omits H12 and H14 entirely from the published matrix/history; H6/H9/H16 remain active Tuesday-Friday and H3 remains disabled.
- Advanced local H1 to v75. GBPUSD and EURUSD H9/H12/H14/H16 now copy GBPCAD's exact entry hour and already-final BUY/SELL for the same block, while retaining the existing GBPUSD-driven pattern/evidence window. Evidence on Web/native iOS/native Android identifies `GBPCAD Hxx` as FINAL SOURCE with `SYNC GBPCAD`; H16 does not apply a second GBPUSD/EURUSD H14 override after copying GBPCAD H16.
- Fixed the stale web Evidence `Copy evidence` action. It is now `Copy chart`, rasterizes the current styled M15 SVG into `image/png` with compact metadata, and writes the image to the system clipboard for Telegram paste while preserving oldest → newest chart chronology.
- Unified the web header/favicon/PWA brand icon with the current native OAK app artwork. `NavBar` now renders `/oak-app-icon.png` instead of the legacy chart favicon, metadata points to the same icon, and the web manifest uses generated 192/512 variants from the canonical AppIcon source.
- Fixed Android launcher adaptive-icon composition: the opaque square iOS artwork is no longer used as the adaptive foreground. Android now uses a navy mask background plus a transparent color foreground regenerated by `tools/sync_brand_assets.py`, while monochrome/themed icons remain intact.

...
- Reworked the H1 core as state v53 / public feed schema 15 / signal rule 48 while preserving all six patterns and entry-time offsets. The authoritative direction is now the exact entry M5 Open versus Bollinger Middle(20), computed from that Open plus 19 prior closed M5 Closes to avoid look-ahead. Above maps XAUUSD/AUDUSD to BUY and the other FX pairs to SELL; below maps the opposite. Missing/equal/incomplete evidence fails closed. The first-Thursday monthly phase now covers all five symbols, with AUDUSD Tuesday and GBPUSD Thursday applied as additional XOR toggles; every net-inverted row is colored in the table and PNG. The explicit `/approve ID` broker-mutation boundary is unchanged.
...
- Preserved valid H1 Pattern 2 alerts when entry-relative M15 evidence is flat; once a pattern supplies its entry time, only the authoritative M5 Open versus Middle(20) evidence gates publication. The H1 table now sizes itself from the active seven hour columns instead of stretching across the legacy 79rem grid.

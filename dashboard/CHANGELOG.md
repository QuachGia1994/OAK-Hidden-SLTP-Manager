...
- H1 signal rule v10/state v16 makes target base polarity explicit while preserving the H3/H4 early slots, H5/H6/H7 pair gates, H8+ fallback allowTrade, DST handling and Thursday/Friday post-signal pipeline. EURUSD, AUDUSD and USDCAD now reverse their own H1 base signal before allowTrade/post-signal; XAUUSD and USDJPY keep their configured base signal unchanged. FX H5/H6 still inspect GBPUSD H2→H1 and XAUUSD H6/H7 still inspect XAUUSD H3→H2, with `TG`/`GT` blocking and `TT`/`GG` remaining normal. Public schema stays 7 and older signal-rule feeds are rejected under rule v10.

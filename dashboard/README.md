...
Current public schema: v18. Cloud state is schema-stable v56 and signal-rule version is v87.

H1 rule v87 keeps the five-row six-block matrix (`XAUUSD`, `GBPUSD`, `GBPAUD`, `GBPCAD`, `GBPJPY`; `H3/H6/H9/H12/H14/H16`) Monday-Friday with XAUUSD as the sole pattern/entry-time owner. On H3-H14, XAUUSD reads the previous available broker day's GBPUSD H(entry-1), GBPAUD reads previous-day AUDUSD, GBPCAD previous-day USDCAD and GBPJPY previous-day USDJPY. Monday therefore resolves the prior Friday. GBPUSD alone keeps the v86 same-broker-day own GBPUSD H(entry-1) rule and waits for that H1 candle to close. H16 remains entry/evidence only. Evidence labels mapped inputs `PREV H(entry-1)` and GBPUSD `OWN H(entry-1)`. History requires a source-bar rebuild because stale v86 mapped rows are rejected rather than relabeled. Telegram signal-slot mapping and broker execution remain independent.

...
- Dashboard/API chỉ nhận record slot active có `logic_version >= 53`, để card theo contract cũ không lẫn vào lịch hiện tại hoặc lịch sử.
- Signal `deactivated` chỉ dùng để đối chiếu; gồm H3 mọi Thứ Năm và H4 mọi ngày.
- Mọi slot H3/H4/H6/H9/H12/H14/H16 phát signal Broker tại `H:00`. Signal XAUUSD lấy từ hai H1 GBPUSD đã đóng của ngày hôm qua ngay trước cùng mốc (H9 hôm nay dùng H8/H7 hôm qua, H8 là nền): hai H1 ngược chiều → BT và giữ hướng nền; hai H1 cùng chiều → SW và đảo hướng nền. Hai H1 GBPAUD tương ứng chỉ dùng để so sánh hướng.
- Nếu hai hướng GBP giống nhau, entry là `H:11`. Nếu ngược nhau, bỏ M15 ngay sát mốc và dùng ba M15 XAUUSD đã đóng kế tiếp của hôm nay (H9 bỏ `08:45`, dùng `08:30/08:15/08:00`): nhóm SW → `(H+1):25`, nhóm BT → `H:49`. Riêng H3: SW → `04:49`, BT → `03:49`.
- DOJI lùi thêm một nến; thiếu dữ liệu hoặc không phân loại được → WAIT. H4 luôn deactivated; H3 mọi Thứ Năm chỉ để tham khảo.

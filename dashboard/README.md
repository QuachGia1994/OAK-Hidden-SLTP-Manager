...
Current public schema: v18. Cloud state is schema-stable v56 and signal-rule version is v85.

H1 rule v85 is a six-block local ICMarkets M15 scanner: `H3/H6/H9/H12/H14/H16`. Monday excludes GBPAUD, GBPCAD and GBPJPY from H1 calculation on every block; XAUUSD keeps normal Monday calculation, while GBPUSD/EURUSD retain their existing H9/H12/H14/H16 eligibility. Tuesday-Friday, GBPAUD, GBPCAD and GBPJPY calculate H3/H6/H9/H12/H14/H16 using AUDUSD, USDCAD and USDJPY M15 respectively, so each cross keeps its own pattern family and SW/BT entry hour. On H3-H14 their final BUY/SELL base is previous-broker-day GBPUSD at H(entry-2); for example entry H8 reads GBPUSD H6. XAUUSD remains XAUUSD scanner + previous-broker-day GBPUSD H(entry-2) on signal-bearing blocks. GBPUSD/EURUSD still copy the exact XAUUSD entry hour/final BUY/SELL only on H9/H12/H14. H16 is pattern/evidence + entry-time only for every eligible row: base direction, scheduled side and BUY/SELL are null/blank, and Evidence labels the rule `ENTRY ONLY`. History preserves H16 entry metadata while sanitizing stale H16 signal fields and stale Monday GBP-cross alerts. Telegram signal-slot mapping still ends at H14 and broker-order execution remains independent from the H1 display matrix.

...
Current public schema: v18. Cloud state is schema-stable v56 and signal-rule version is v99.

H1 rule v99 exposes one shared `ENTRY TIME` row plus GBPAUD across four active blocks (`H3/H6/H9/H12`) Monday-Friday; H14 is removed as a block, but H12 may still resolve to entry H14. XAUUSD is the sole pattern/entry-time owner: only H3 is evaluated for the entry delta. H3 entry H4 cascades H6/H7, H9/H10 and H12/H13; H3 entry H5 cascades H6/H8, H9/H11 and H12/H14. GBPAUD reads its own same-day H1 candle at `entry time - 1`; `T` maps to BUY and `G` to SELL with no delta inversion and no previous-block/previous-day selector. Missing exact GBPAUD H1 evidence fails closed. GBPUSD remains an internal M15 source for XAUUSD private TP-roll handling only. Telegram signal-slot mapping and broker execution remain independent.

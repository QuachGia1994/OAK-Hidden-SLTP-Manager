...
Current public schema: v18. Cloud state is schema-stable v56 and signal-rule version is v83.

H1 rule v83 is a five-block local ICMarkets M15 scanner: `H3/H6/H9/H12/H14`. H16 is retired and is neither calculated nor published. Monday uses the same normal weekday eligibility as Tuesday-Friday: GBPAUD is eligible on H3/H6/H9/H12/H14, GBPUSD/EURUSD/GBPCAD on H9/H12/H14, and GBPJPY on H6/H9 only. GBPAUD, GBPCAD and GBPJPY scan AUDUSD, USDCAD and USDJPY M15 respectively, so each cross gets its own pattern family and SW/BT entry hour. Their final BUY/SELL base is previous-broker-day GBPUSD at H(entry-2); for example entry H8 reads GBPUSD H6. XAUUSD remains XAUUSD scanner + previous-broker-day GBPUSD H(entry-2). GBPUSD/EURUSD H9/H12/H14 still copy the exact XAUUSD entry hour and final BUY/SELL for the same block. History uses the schema-stable state key and strips legacy H16 rows during parse/backfill. Telegram broker-order execution remains independent from the H1 display-slot map.

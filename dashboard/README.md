...
Current public schema: v18. Cloud state is schema-stable v56 and signal-rule version is v90.

H1 rule v90 keeps five rows (`XAUUSD`, `GBPUSD`, `AUDUSD`, `USDCAD`, `USDJPY`) and five active blocks (`H3/H6/H9/H12/H14`) Monday-Friday; H16 is removed. XAUUSD remains the sole pattern/entry-time owner. Each row first reads its own same-broker-day M15 candle at `entry time - 2h15`; XAUUSD/GBPUSD/AUDUSD keep the resulting BUY/SELL while USDCAD/USDJPY invert it. A final H3-entry selector then applies: H3/H6/H9 use the previous available broker day's XAUUSD H3 Entry (`H4` keep, `H5` invert), and H12/H14 use today's H3 Entry with the same rule. Missing selector evidence fails closed. Local snapshots include the previous XAUUSD broker day only for this selector; signal bases remain own-symbol/same-day. Stale v89 local-pattern rows are rejected and History requires a 90-day source rebuild. Telegram signal-slot mapping and broker execution remain independent.

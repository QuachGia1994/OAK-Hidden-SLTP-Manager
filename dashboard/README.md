...
Current public schema: v18. Cloud state is schema-stable v56 and signal-rule version is v88.

H1 rule v88 keeps the five-row six-block matrix (`XAUUSD`, `GBPUSD`, `GBPAUD`, `GBPCAD`, `GBPJPY`; `H3/H6/H9/H12/H14/H16`) Monday-Friday with XAUUSD as the sole pattern/entry-time owner. On H3-H14, every signal base now uses H(entry-2): XAUUSD reads the previous available broker day's GBPUSD, GBPAUD reads previous-day AUDUSD, GBPCAD previous-day USDCAD and GBPJPY previous-day USDJPY; Monday therefore resolves the prior Friday. GBPUSD keeps same-broker-day own GBPUSD but also uses H(entry-2). H16 remains entry/evidence only. Evidence labels mapped inputs `PREV H(entry-2)` and GBPUSD `OWN H(entry-2)`. History requires a source-bar rebuild because stale v87 H(entry-1) rows are rejected rather than relabeled. Telegram signal-slot mapping and broker execution remain independent.

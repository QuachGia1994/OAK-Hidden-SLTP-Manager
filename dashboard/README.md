...
Current public schema: v16. Cloud state is v54 and signal-rule version is v49.

The H1 engine routes `H3` to the four FX targets (`GBPUSD/AUDUSD/USDCAD/USDJPY`), `H4` to `XAUUSD` only, and `H6/H9/H12/H14/H16` to all five targets. All six pattern classifiers and entry-time offsets remain intact: P1/P2/P5 enter at `H+2:00`; P3/P4 enter at `H+1:25`; P6 uses pattern candles 5-6 to choose `H+2:00` for `TG/GT` or `H+1:25` for `TT/GG`. The final signal base is the broker H1 candle one hour before the entry time, so `08:00` and `08:25` both use `H07:00`; its bullish/bearish direction maps directly to BUY/SELL for every FX pair and XAUUSD. The cycle-month six-block matrix is shared by all symbols: Thu/Fri invert H3/H4 and H12/H16 while keeping H6/H9/H14; Tue/Wed keep H3/H4 and H12/H16 while inverting H6/H9/H14; Monday inverts H3/H4 and H6/H9/H14 while keeping H12/H16. A non-cycle Thursday month uses the inverse matrix. Historical records, PNG and detail modal show the H1 base candle; M5 remains transport data only and cannot veto a classified alert.

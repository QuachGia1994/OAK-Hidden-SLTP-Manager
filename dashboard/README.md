...
Current public schema: v16. Cloud state is v54 and signal-rule version is v50.

The H1 engine routes `H3` to the four FX targets (`GBPUSD/AUDUSD/USDCAD/USDJPY`), `H4` to `XAUUSD` only, and `H6/H9/H12/H14/H16` to all five targets. All six pattern classifiers and entry-time offsets remain intact: P1/P2/P5 enter at `H+2:00`; P3/P4 enter at `H+1:25`; P6 uses pattern candles 5-6 to choose `H+2:00` for `TG/GT` or `H+1:25` for `TT/GG`. The final signal base is the broker H1 candle one hour before the entry time, so `08:00` and `08:25` both use `H07:00`; its bullish/bearish direction maps directly to BUY/SELL for every FX pair and XAUUSD. The cycle-month six-block post-signal matrix is shared by all symbols; blocks are `[H3/H4] [H6] [H9] [H12] [H14] [H16]` and each block decides independently (`N` = inverted post-signal, `C` = keep): Monday `C N N C C C`, Tuesday `N C N C N C`, Wednesday `N C C C N C`, Thursday `N C C N C N`, Friday `N C C N C C`. A non-cycle (regular) month uses the exact inverse of the weekday row. Historical records, PNG and detail modal show the H1 base candle; M5 remains transport data only and cannot veto a classified alert.

...
test("rule v87 history is rebuilt from local ICMarkets M15 snapshots, not legacy cTrader H1 reconstruction", () => {
...
test("local history publisher is bounded to 90 calendar days and carries current plus previous broker-day mapped base bars", () => {
  ...
  assert.match(publisher, /AUDUSD/);
  assert.match(publisher, /USDCAD/);
  assert.match(publisher, /USDJPY/);

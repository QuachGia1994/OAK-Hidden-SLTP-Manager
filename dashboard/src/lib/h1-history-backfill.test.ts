...
test("rule v86 history is rebuilt from local ICMarkets M15 snapshots, not legacy cTrader H1 reconstruction", () => {
...
test("local history publisher is bounded to 90 calendar days and uses same-day displayed-symbol bars only", () => {
  assert.match(publisher, /MAX_BACKFILL_DAYS = 90/);
  assert.doesNotMatch(publisher, /previousAvailableDate|PREVIOUS_DAY_BASE_SOURCES|snapshotBarsForSource/);

...
export const RULES_BY_LOCALE = {
  "VN": [
    ...
    "Live execution chạy Thứ Hai đến Thứ Sáu; cuối tuần chỉ chạy live khi profile đặt signal_live_weekends=true. Rebuild/backtest chạy 24/7: slot cuối tuần được đánh giá từ bar MT4 Feed đã lưu (READY) hoặc báo WAIT_MT4_DATA/thiếu dữ liệu — không bỏ qua ngày, không mượn nến Thứ Sáu.",
    ...
    "Cài MT4 Feed v87: chạy server trước, đặt FeedBaseURL của MT4_Data_Feeder là http://127.0.0.1/mt4-feed (cổng HTTP mặc định 80), rồi cho phép http://127.0.0.1 trong WebRequest của MT4. Cổng :5001 chỉ giữ cho health/management nội bộ. Gắn EA vào mọi chart cần lưu raw bars; EA tự nhận diện Symbol(), cả tiền tố/hậu tố broker; core Signal v87 vẫn cần XAUUSD/GOLD, GBPUSD, GBPAUD, GBPJPY, GBPCAD; không có SymbolName thủ công.",
    "Endpoint cũ http://127.0.0.1:5000/mt4_data và EA có các input ServerURL/BrokerName/SymbolName/MagicNumber không phải feed v87."
  ],
  "EN": [
    ...
    "Live execution runs Monday through Friday; weekend live slots require signal_live_weekends=true in the active profile. Rebuild/backtest runs 24/7: weekend slots are evaluated from the persisted MT4 Feed bars (READY) or report WAIT_MT4_DATA/missing data — never skipped, never filled with Friday candles.",
    ...
    "MT4 Feed v87 setup: start the server first, set MT4_Data_Feeder FeedBaseURL to http://127.0.0.1/mt4-feed (default HTTP port 80), and allow http://127.0.0.1 in MT4 WebRequest permissions. Port :5001 remains internal health/management only. Then attach the EA to any chart that should persist raw bars. The EA auto-detects Symbol(), including broker prefixes/suffixes; the v87 Signal core still requires XAUUSD/GOLD, GBPUSD, GBPAUD, GBPJPY, and GBPCAD; no manual SymbolName is used.",
    "The legacy http://127.0.0.1:5000/mt4_data endpoint and an EA with ServerURL/BrokerName/SymbolName/MagicNumber inputs are not the v87 feed."
  ]
} as const;

export const STARTUP_SUMMARY_BY_LOCALE = "v88: weekend-capable 24/7 rebuild, slot-scoped active pairs, Final Reverse on every applicable pair, XAUUSD==GBPUSD invariant" as const;

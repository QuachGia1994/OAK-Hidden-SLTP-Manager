...
test("rule v82 uses local MT5 ICMarkets, schema 18 and five blocks with H16 retired", () => {
  assert.equal(H1_CLOUD_STATE_VERSION, 56);
  assert.equal(H1_PUBLIC_SCHEMA, 18);
  assert.equal(H1_SIGNAL_RULE_VERSION, 82);
...
test("GBPAUD pattern timing comes from GBPUSD while final base comes from AUDUSD H(entry-2)", () => {
  const date = "2026-09-02";
  const snapshot = market(date, "TTGTTT", "ALT");
  snapshot.AUDUSD.bars = [...snapshot.AUDUSD.bars, ...h1Bars("2026-09-01", 2, "T")];
...
test("Tuesday GBPAUD uses GBPUSD scanner and AUDUSD H(entry-2) signal base", () => {
  const date = "2026-09-08";
  const snapshot = market(date, "TTGTTT", "ALT");
  snapshot.AUDUSD.bars = [...snapshot.AUDUSD.bars, ...h1Bars("2026-09-07", 2, "G")];
...
test("GBPUSD and EURUSD copy XAUUSD entry time and final side on H9/H12/H14", () => {
  for (const [date, previousDate] of [
    ...
    for (const slotHour of [9, 12, 14] as const) {
      ...
      snapshot.USDCAD.bars = [...snapshot.USDCAD.bars, ...h1Bars(previousDate, slotHour - 1, "G")];
      ...
      assert.equal(cad?.scannerSource, "GBPUSD");
      assert.equal(cad?.baseSymbol, "USDCAD");
...
test("GBP crosses share GBPUSD scanner timing with dedicated H(entry-2) signal bases", () => {
  const date = "2026-09-02";
  const previousDate = "2026-09-01";
  const configs = [
    ["GBPAUD", "AUDUSD", "BUY", [3, 6, 9, 12, 14]],
    ["GBPCAD", "USDCAD", "SELL", [9, 12, 14]],
    ["GBPJPY", "USDJPY", "BUY", [6, 9]],
  ] as const;
  ...
  for (const slotHour of H1_SCAN_HOURS) {
    ...
    const snapshot = market(date, "TTGTTT", "ALT");
    snapshot.GBPUSD.bars = bars(date, "TTGTTT", "ALT").map((row) => ({ ...row, hour: row.hour + shift }));
    snapshot.AUDUSD.bars = [...snapshot.AUDUSD.bars, ...h1Bars(previousDate, slotHour - 1, "T")];
    snapshot.USDCAD.bars = [...snapshot.USDCAD.bars, ...h1Bars(previousDate, slotHour - 1, "G")];
    snapshot.USDJPY.bars = [...snapshot.USDJPY.bars, ...h1Bars(previousDate, slotHour - 1, "T")];
...
test("shared GBPUSD scanner gives crosses one entry while dedicated bases can diverge", () => {
  const date = "2026-09-02";
  const previousDate = "2026-09-01";
  const slotHour = 9;
  const shift = slotHour - 3;
  const snapshot = market(date, "TTGTTT", "ALT");
  snapshot.GBPUSD.bars = bars(date, "TGGTTT", "ALT").map((row) => ({ ...row, hour: row.hour + shift }));
  snapshot.AUDUSD.bars = [...snapshot.AUDUSD.bars, ...h1Bars(previousDate, 9, "T")];
  snapshot.USDCAD.bars = [...snapshot.USDCAD.bars, ...h1Bars(previousDate, 9, "G")];
  snapshot.USDJPY.bars = [...snapshot.USDJPY.bars, ...h1Bars(previousDate, 9, "T")];
  ...
  assert.deepEqual([aud?.scannerSource, aud?.entryHour, aud?.baseSymbol, aud?.baseHour, aud?.symbolH1Signal], ["GBPUSD", 11, "AUDUSD", 9, "BUY"]);
  assert.deepEqual([cad?.scannerSource, cad?.entryHour, cad?.baseSymbol, cad?.baseHour, cad?.symbolH1Signal], ["GBPUSD", 11, "USDCAD", 9, "SELL"]);
  assert.deepEqual([jpy?.scannerSource, jpy?.entryHour, jpy?.baseSymbol, jpy?.baseHour, jpy?.symbolH1Signal], ["GBPUSD", 11, "USDJPY", 9, "BUY"]);
});
...
test("Monday evaluates shared GBPUSD FX scanner with dedicated cross bases like normal weekdays", () => {
  const monday = "2026-09-07";
  const previousFriday = "2026-09-04";
  const configs = [
    ["GBPUSD", "GBPUSD", "BUY", [9, 12, 14]],
    ["EURUSD", "GBPUSD", "BUY", [9, 12, 14]],
    ["GBPAUD", "AUDUSD", "BUY", [3, 6, 9, 12, 14]],
    ["GBPCAD", "USDCAD", "SELL", [9, 12, 14]],
    ["GBPJPY", "USDJPY", "BUY", [6, 9]],
  ] as const;
  ...
  for (const slotHour of H1_SCAN_HOURS) {
    ...
    snapshot.AUDUSD.bars = [...snapshot.AUDUSD.bars, ...h1Bars(previousFriday, slotHour - 1, "T")];
    snapshot.USDCAD.bars = [...snapshot.USDCAD.bars, ...h1Bars(previousFriday, slotHour - 1, "G")];
    snapshot.USDJPY.bars = [...snapshot.USDJPY.bars, ...h1Bars(previousFriday, slotHour - 1, "T")];
...
test("cloud state v56 round-trips the v82 GBP-cross scanner/base contract", () => {
  const date = "2026-09-02";
  const state = emptyCloudState();
  const snapshot = market(date, "TTGTTT", "ALT");
  snapshot.AUDUSD.bars = [...snapshot.AUDUSD.bars, ...h1Bars("2026-09-01", 2, "G")];
  const alert = evaluateLocalH1PatternsForTarget("GBPAUD", date, snapshot, [3], 3)[0];
...
test("legacy history merges under v82 without losing dates and strips retired H16 rows", () => {
...
test("public feed schema 18 exposes entry time plus final BUY/SELL and can seed state", () => {
  ...
  ensureSymbolDay(state, date, "GBPAUD").symbol.alerts.push(alert);
  const feed = buildPublicFeed(state, "2026-09-02T01:00:00.000Z");
  assert.deepEqual([feed.schemaVersion, feed.signalRuleVersion, feed.hours], [18, 82, [3, 6, 9, 12, 14]]);

        ...
        <div ref={tableScrollRef} className="oak-h1-table-scroll lux-scroll">
          <table className="oak-h1-table">
            ...
            <tbody>
              ...
              {H1_SIGNAL_ROWS.map((symbol) => <tr key={symbol}><th id={`h1-signal-row-${symbol}`} scope="row" className="oak-h1-symbol-sticky"><b>{symbol}</b><small>{symbol === "USDCAD" || symbol === "USDJPY" ? "M15 E-2:15 · INVERT" : "M15 E-2:15 · KEEP"}</small></th>{fallbackHours.map((hour) => (
        ...
        {!date ? <div className="oak-empty-state oak-h1-history-empty"><span>∅</span><p>{copy.noMatch}</p></div> : <><p className="oak-h1-scroll-hint">{locale === "EN" ? "Swipe if the table extends beyond the screen" : "Vuốt ngang nếu bảng rộng hơn màn hình"}</p><div ref={tableScrollRef} className="oak-h1-table-scroll lux-scroll">
          <table className="oak-h1-table">
            ...
            <tbody>
              ...
              {H1_SIGNAL_ROWS.map((symbol) => <tr key={symbol}><th id={`h1-signal-row-${symbol}`} scope="row" className="oak-h1-symbol-sticky"><b>{symbol}</b><small>{symbol === "USDCAD" || symbol === "USDJPY" ? "M15 E-2:15 · INVERT" : "M15 E-2:15 · KEEP"}</small></th>{activeHours.map((hour) => {
                const alert = signalAlertForHour(day, symbol, hour);
                const signal = signalLabel(alert);

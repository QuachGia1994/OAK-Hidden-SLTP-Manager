      ...
      <header className="terminal-hero rules-hero overflow-hidden rounded-xl">
        <div className="relative grid lg:grid-cols-[minmax(15rem,0.85fr)_minmax(0,2.15fr)]">
          <div className="flex min-h-32 flex-col justify-center border-b border-[color:var(--panel-border)] px-5 py-5 lg:border-b-0 lg:border-r lg:px-6">
            ...
            <h1 className="break-words text-4xl font-black tracking-tight text-[var(--foreground)] sm:text-5xl">
              {t.ruleList}
            </h1>
          </div>
          ...
          <ol className="rule-list">
            {todayRules.map((rule, index) => (
              <li key={`${index}-${rule}`} className="rule-row">
                ...
                <p className="min-w-0 text-sm font-medium leading-6 text-[var(--foreground)]">{rule}</p>
                <span className="rule-row-marker" aria-hidden="true" />
              </li>
            ))}
          </ol>
        ) : (
          <div className="rule-empty-state px-5 py-12 text-center text-sm font-medium text-[var(--muted)]">
            {t.noRule}
          </div>
        )}
      </section>
...
function RuleMeta({
  ...
  return (
    <div className={`rule-meta px-5 py-4 ${live ? "rule-meta-live" : ""}`}>
      ...
      <div className="break-words text-sm font-black leading-6 text-[var(--foreground)]">{value}</div>
      {subLabel && <div className="mt-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--muted)]">{subLabel}</div>}
    </div>
  );
}

...
- Entry netting policy: scheduled same-direction milestones roll the existing TP instead of opening another position; XAU/GOLD advances by 20.0 price units and FX by 50 pips. Opposite direction still closes opposite positions and removes opposite pending orders before a new entry. EA v1.14 adds a separate H1 `tp_roll` action driven by calculated Entry Time; this action can only roll one same-direction TP or no-op and never opens/net-closes a position. EA v1.13 introduced the scheduled-entry TP roll and preserves the v1.12 projected post-net exposure preflight and `REVERSAL_INCOMPLETE` evidence.
...
- EA v1.08 adds internal `entry_prepare`: it preserves opposite-position/pending cleanup, exposure/lot/symbol/tick guards and absolute SL/TP calculation, then returns exact fields for the controller. EA v1.12 additionally returns net-mutation/current-exposure evidence. EA v1.13 changes the due scheduled same-direction case: when exactly one same-direction position is still open, it preserves SL, advances the current TP by one fixed step and returns a durable `tpRolled` result instead of submitting another Buy/Sell. EA v1.14 adds the independent `tp_roll` mutation used by H1-calculated Entry-Time milestones; its canonical `h1tp:` origin is durable/replay-fenced and the action never enters or nets a trade. If entry netting changed exposure but the replacement UI order is not submitted, the controller reports `REVERSAL_INCOMPLETE`, requires manual broker inspection, and never auto-replays the order. Immediate entries and all management actions remain EA-executed. No global mouse or keyboard injection is used.

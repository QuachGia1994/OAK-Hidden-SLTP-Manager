...
- Reworked the H1 signal engine as state v50 / feed schema 12 / rule 44. P1 and P4 use pattern candle 3 unchanged; P3 uses pattern candle 3 inverted; P2 keeps the live `H:00` base and P5 inverts the post-block `H:15` base. Pattern 6 requires a six-candle window whose first four candles are `TGTG/GTGT`, keeps candle 5 as its base, enters at `H+2:00` for pair 5-6 `TG/GT` and at `H+1:25` for `TT/GG`, and renders a manual-decision warning. Thursday GBPUSD and Tuesday AUDUSD still invert once after the pattern result. The XAU Thursday/Friday/Monday cycle remains visual metadata only. Routing and explicit `/approve ID` safety are unchanged.

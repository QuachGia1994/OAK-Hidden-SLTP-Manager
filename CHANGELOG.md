...
- Reworked the H1 signal engine (state v47 / feed schema 9 / rule 41) around the post-block M15 base. The two pre-block M15 candles select only the Pattern 1-5 window; P1/P3/P4/P5 use the block's `H:15` candle, P1/P5 invert that base, P3/P4 keep it and enter at `H+1:25`, while P2 uses and keeps `H:00` for its `H:01` entry. H3 remains FX-only, H4 remains XAUUSD-only and H6/H9/H12/H14/H16 cover all five targets. Cloudflare/GitHub phases now evaluate the live `H:00/H:01/H:30` boundaries needed by those bases. Eligible live signals create one idempotent `approval_required` cTrader intent for the scanner account with lot `0.03`; Telegram shows side, symbol, time, account and `/approve ID`, and no broker execution occurs before explicit approval. The `/engine` table continues to expose BUY/SELL, and NeoTech Master pairing now uses an accessible in-page risk-confirmation dialog instead of a browser-native confirm.
- Simplified the H1 scanner to the routed block grid (`H3` FX, `H4` XAUUSD, then `H6/H9/H12/H14/H16` all targets) and five pattern families: `TGG/GTT`, `TTT/GGG`, `TGT/GTG`, `GGT/TTG`, and 4+ same-direction candles. Removed legacy cross-symbol inheritance, H1-base mapping, lookback gates and BLOCK/not-trade states; the retained Thursday/Friday calendar cycle remains a final XAUUSD-only adjustment.

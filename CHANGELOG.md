...
- H1 signal rule v10 keeps public schema 7 and moves cloud state to v16. Target polarity is now explicit before allowTrade and calendar post-signal: EURUSD, AUDUSD and USDCAD invert their own H1 base signal; XAUUSD and USDJPY keep their configured base signal unchanged. The existing target pair gates remain intact: FX H5/H6 inspect GBPUSD H2→H1, XAUUSD H6/H7 inspect XAUUSD H3→H2, `TG`/`GT` blocks and `TT`/`GG` leaves trade eligibility normal. XAUUSD H5 remains inactive, FX H7 has no pair gate, generic SW3 remains H06-H16, H8+ allowTrade is unchanged, and Thursday/Friday special post-signal still applies last.

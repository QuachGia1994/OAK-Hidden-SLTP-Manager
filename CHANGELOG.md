...
- H1 signal rule v5 preserves the existing scanner-source/base-H1/calendar post-signal transform, keeps public schema 7, moves cloud state to v11, and rejects older signal-rule feeds so stale allowTrade semantics cannot be relabeled as current state.
- Replaced the H1 pure-only cooldown with signal-rule v5/state v11 allowTrade lookback: from H8 onward scanner Pattern 1 (`TGG`/`GTT`) inspects the prior non-overlapping three candles (H4/H3/H2 at H8); prior Pattern 1 or Pattern 2 (`TTT`/`GGG`) becomes `BLOCK / NOT TRADE`, while prior Pattern 3 (`GTG`/`TGT`) reverses the calculated signal once before the independent calendar post-signal step. Scanner Pattern 2 remains the strongest pattern and skips this lookback.

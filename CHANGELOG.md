...
- Reworked the H1 signal engine as state v52 / feed schema 14 / rule 46. Pattern classification and the entry-relative M15 signal pair are unchanged. XAUUSD alone now uses a calendar-month post-signal phase anchored by the month's first Thursday: a cycle first Thursday keeps Thu/Mon/Wed and reverses Fri/Tue, while a regular first Thursday reverses Thu/Mon/Wed and keeps Fri/Tue; the selected phase repeats across later weeks. Only reversed XAU dates color the XAU row. GBPUSD still reverses Thursday, AUDUSD still reverses Tuesday, and USDCAD/USDJPY remain unchanged. Intents stay `approval_required` and never execute without explicit `/approve ID`.

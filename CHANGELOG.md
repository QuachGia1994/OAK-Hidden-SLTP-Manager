...
- H1 signal rule v7 keeps public schema 7, moves cloud state to v13, and adds early two-candle slots without changing the H8+ allowTrade contract: FX targets scan the GBPUSD two-candle pattern only at H3 and follow their own H2 base; XAUUSD scans its own two-candle pattern only at H4 and follows GBPUSD H3. `TG`, `GT`, `TT`, and `GG` are all valid two-candle patterns, remain always trade-eligible, and keep the base signal before the existing Thursday/Friday special post-signal inversion. H5 remains inactive and the SW3 scanner remains H06-H16.

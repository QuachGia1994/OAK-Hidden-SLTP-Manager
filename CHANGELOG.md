...
- Extended the H1 cloud scanner/feed to retain 90 calendar days of broker history, added an admin/API-authenticated cTrader historical backfill that reconstructs missing dates with the existing signal-rule v4 without Telegram/trading side effects, and added weekday/date history navigation to `/engine` while preserving schema 7, state v10, VIP redaction and mobile latest-day behavior.

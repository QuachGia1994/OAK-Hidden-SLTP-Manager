...
- Reworked the H1 signal engine as state v51 / feed schema 13 / rule 45. Pattern classification remains independent from signal refinement: P1/P2/P5 enter at `H+2:00`, P3/P4 at `H+1:25`, and P6 keeps its pair-5/6 choice between those times. For every `:00` entry the engine uses M15 `entry-15` then `entry-30`; for every `:25` entry it uses `entry-25` then `entry-40`. A same-direction pair keeps candle 1 and an alternating pair reverses candle 1. H:15 scheduling and M15-close readiness checks prevent late or empty table rows. Thursday GBPUSD and Tuesday AUDUSD still invert once after this result; the XAU Thursday/Friday/Monday cycle remains visual metadata. Intents remain `approval_required` and never execute without explicit `/approve ID`.

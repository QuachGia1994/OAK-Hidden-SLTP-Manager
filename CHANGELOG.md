...
- H1 signal rule v3 keeps the v2 source/base/calendar transform unchanged while moving cloud/local state to v9 and public `signalRuleVersion=3`; schema-v7 rule-v2 feeds remain migration input so a cutover does not blank or replay the current day.
- A tradeable pure SW3 now starts a three-slot cooldown: H(n+1)..H(n+3) remain fully scanned/calculated but render `BLOCK / NOT TRADE` on the web and do not emit actionable Telegram alerts. Pure SW3 found inside cooldown is retained as blocked evidence and does not extend the cooldown; H(n+4) is active again (for example H12 pure → H13-H15 BLOCK → H16 active).

...
- H1 signal rule v11 keeps public schema 7 and moves cloud state to v17. H5 is now excluded as a scan/signal slot for every target: it is not calculated, stored, backfilled, published or pair-gated. Active slots return to H3/H4 plus H06-H16. FX pair-gating now applies only at H6 using GBPUSD H2→H1; XAUUSD pair-gating remains H6/H7 using XAUUSD H3→H2. Target polarity is unchanged from v10: EURUSD, AUDUSD and USDCAD invert their own H1 base signal, while XAUUSD and USDJPY keep their configured base signal. H8+ allowTrade may still use the H5 candle as lookback evidence, and Thursday/Friday special post-signal still applies last.

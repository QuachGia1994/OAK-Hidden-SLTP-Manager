...
- Fixed H1 Pattern 2 publication so flat entry-relative M15 evidence no longer discards a pattern before the authoritative M5 Open versus Middle(20) check; the dashboard table now sizes to its active seven hour columns instead of the legacy 79rem grid.
...
- Reworked the H1 signal engine as state v53 / feed schema 15 / rule 48 while preserving all six patterns and their existing entry times. Signal direction now comes from the exact entry M5 Open against Bollinger Middle(20), period 20 / shift 0 / applied Close without historical look-ahead: above maps XAUUSD/AUDUSD to BUY and GBPUSD/USDCAD/USDJPY to SELL; below maps the opposite. Missing, equal, non-finite or incomplete M5 evidence produces no alert. The first-Thursday monthly phase now applies to all five symbols, then AUDUSD Tuesday and GBPUSD Thursday each toggle once more; XOR composition means double reversals cancel. Every symbol row with a final net reversal is highlighted. Intents stay `approval_required` and never execute without explicit `/approve ID`.

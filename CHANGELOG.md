...
- H1 signal rule v9 keeps public schema 7, moves cloud state to v15, and extends the target-specific pair gates. FX targets keep their H3 GBPUSD two-candle scanner and own H2 base, now also scan SW3 at H5, and apply the GBPUSD H2→H1 pair gate at both H5 and H6: `TG`/`GT` blocks while `TT`/`GG` leaves the signal unchanged. XAUUSD keeps its H4 own-symbol two-candle scanner with GBPUSD H3 base and applies the XAUUSD H3→H2 pair gate at both H6 and H7 with the same block/normal rule. XAUUSD H5 remains inactive and FX H7 has no pair gate. The generic SW3 scanner remains H06-H16, H8+ allowTrade remains unchanged, and the retired pure-slot cooldown/following-slot block is not part of the current engine.

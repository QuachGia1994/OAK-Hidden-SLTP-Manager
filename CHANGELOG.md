...
- H1 signal rule v8 keeps public schema 7, moves cloud state to v14, and adds target-specific early pair gates on top of the existing H3/H4 two-candle slots and H8+ allowTrade contract. FX targets keep their H3 GBPUSD two-candle scanner and own H2 base, then at H6 inspect GBPUSD H2→H1: `TG`/`GT` blocks while `TT`/`GG` leaves the H6 signal unchanged. XAUUSD keeps its H4 own-symbol two-candle scanner with GBPUSD H3 base, then at H7 inspects XAUUSD H3→H2 with the same `TG`/`GT` block and `TT`/`GG` normal rule. XAU H6 and FX H7 are not pair-gated. H5 remains inactive, SW3 remains H06-H16, and the retired pure-slot cooldown/following-slot block is not part of the current engine.

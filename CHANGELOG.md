...
- Rewrote the H1 core engine (state v46 / feed schema 8 / rule 40): each symbol now derives its signal from its own H-1 H1 candle refined by the two M15 candles before the block (TT/GG keeps, TG/GT inverts), the M15 pattern window classifies Pattern 1-5 with Pattern 5 taking precedence whenever the same-direction run reaches four candles, and every alert carries a pattern-driven entry time (`P1 +2:00`, `P2 +0:01`, `P3/P4 +1:35`, `P5 +2:00`). Added the H4 block for XAUUSD only while H3 scans the four FX pairs and H6/H9/H12/H14/H16 scan all five targets; removed all cross-symbol base mapping, lookback gates and BLOCK/not-trade states. The special calendar cycle now applies to XAUUSD only across whole days: a special Thursday (first Friday of the following month's month-start on day 3/4/7, or the prior Wednesday on day 30/1) inverts Thursday, keeps Friday and inverts the next Monday; a normal Thursday keeps Thursday, inverts Friday and keeps Monday. Historical backfill now reconstructs through the same engine from H1+M15 history under a serverless time budget.
- Simplified the H1 scanner to one fixed six-slot grid (`H3/H6/H9/H12/H14/H16`) for every target and five classification-only patterns: `TGG/GTT`, `TTT/GGG`, `TGT/GTG`, `GGT/TTG`, and 4+ same-direction candles. Removed pair/lookback/BLOCK/keep/invert actions, H4 inheritance and special per-hour routing; only the existing Thursday/Friday special calendar cycle may invert the mapped base signal.

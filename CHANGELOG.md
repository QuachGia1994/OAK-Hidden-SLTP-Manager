...
- Reworked the H1 signal engine as state v49 / feed schema 11 / rule 43. Pattern 6 now requires a six-candle window whose first four candles are `TGTG/GTGT`; candle 5 is the unchanged signal base, while pair 5-6 `TG/GT` enters at `H+2:00` and `TT/GG` enters at `H+1:25`. P6 cells carry a warning for manual judgment. P1/P5 still invert, P2 keeps `H:00` for `H:01`, and P3/P4 keep `H:15` for `H+1:25`. Thursday GBPUSD and Tuesday AUDUSD invert once after the pattern result. The XAU Thursday/Friday/Monday cycle remains visual metadata only. Routing and explicit `/approve ID` safety are unchanged.

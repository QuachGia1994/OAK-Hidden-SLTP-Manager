...
- Upgraded `/accounts` from cTrader-only to a provider-neutral account control plane. cTrader live/demo accounts still sync through encrypted server-side OAuth, while MT5 accounts can be registered as non-secret bridge metadata (broker/login/environment/profile). A unified default/enabled account registry now feeds Telegram `/profiles`; browser/API responses never expose OAuth tokens, client secrets, broker passwords or vault material. Existing cTrader execution targeting remains unchanged until MT5 bridge execution is deliberately wired.

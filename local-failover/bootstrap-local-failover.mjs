...
export async function main({ localPrimary = process.argv.includes("--local-primary") } = {}) {
  ...
  if (result.localPrimary) {
    ...
    console.log("Attach EA 1.14+ first: H1 Entry-Time roll-only milestones require tp_roll support; scheduled UI entry also requires reversal-safe entry_prepare evidence and position-comment verification.");
  } else {

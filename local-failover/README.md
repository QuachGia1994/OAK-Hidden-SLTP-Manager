...
H1 rule v88 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the six active blocks `H3/H6/H9/H12/H14/H16`.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route. Rule v88 reads eight market sources: the five displayed rows plus AUDUSD/USDCAD/USDJPY signal bases. XAUUSD alone owns pattern/SW-BT entry-time detection for every row. On H3-H14, every signal base uses H(entry-2): XAUUSD reads previous available broker-day GBPUSD, GBPAUD previous-day AUDUSD, GBPCAD previous-day USDCAD and GBPJPY previous-day USDJPY; Monday resolves Friday. GBPUSD keeps same-day own GBPUSD but also uses H(entry-2). H16 stays entry/evidence only. Live/history snapshots already carry enough current-day and previous available broker-day bars, so no local payload shape change is required. Telegram signal-slot mapping and broker-order scheduling remain separate and unaffected. Stale v87 H(entry-1) rows are rejected rather than relabeled, so History needs a 90-day local source rebuild after deployment.

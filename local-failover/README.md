...
H1 rule v98 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API for retained M15/H1 bars and never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the five active blocks `H3/H6/H9/H12/H14`; H16 remains removed as a block.

The local market-source set remains `XAUUSD` + `GBPUSD` + `GBPAUD`. XAUUSD remains the only pattern/SW-BT detector and shared Entry-time owner; its internal GBPUSD M15 `entry - 15m` signal is retained only for internal XAU TP-roll behavior and is not published as a signal row. The public feed/table now contains only GBPAUD: for every H3/H6/H9/H12/H14 block it reads GBPAUD H1 at `entryHour - 1`; Entry delta `+1` KEEPS the raw base direction and Entry delta `+2` INVERTS it. Missing the exact GBPAUD H1 candle fails closed. GBPUSD is no longer a public signal or public TP-milestone symbol. Each live v98 scan writes current-day TP milestones using the shared XAUUSD Entry time, and the local-primary controller accepts only the matching v98 milestone contract. Public schema 18 and cloud state v56 remain stable; stale v97 public rows are rejected and a valid v98 public replica reconstructs the hidden XAU Entry owner plus the single GBPAUD signal row.

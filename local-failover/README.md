...
H1 rule v68 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating `H3/H6/H9/H12/H14/H16`.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route. Each live/backfill snapshot carries the previous available broker day for every signal-base source: GBPUSD, AUDUSD, USDCAD and USDJPY. Rule v68 gives GBPAUD, GBPCAD and GBPJPY one shared pattern/evidence + entry-time driver: GBPUSD M15, across H3/H6/H9/H12/H14/H16. Their final signal bases are independent: GBPAUD reads AUDUSD H(entry-1), GBPCAD reads USDCAD H(entry-1), and GBPJPY reads USDJPY H(entry-1), all from the previous available broker day; rising => BUY and falling => SELL with no extra GBP-cross inversion. These three GBP crosses are active on all six blocks on weekdays, including Monday. GBPUSD/EURUSD keep their existing later-block behavior and XAUUSD-side synchronization, with Thursday flipping GBPUSD and Friday flipping EURUSD. If the first XAUUSD entry of the day is H5, H16 remains a manual CLOSE-only advisory block and every H16 BUY/SELL signal is suppressed to null while entry/pattern evidence remains available; there is no automatic broker-close path. Legacy Thursday/Friday special propagation and CẦU presentation remain disabled. Legacy cloud scanner/backfill endpoints remain authenticated no-ops, so cTrader cannot overwrite rule-v68 state. History uses the same local source and can be seeded for up to 90 calendar days.

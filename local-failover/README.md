...
H1 rule v86 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the six active blocks `H3/H6/H9/H12/H14/H16`.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route. Rule v86 snapshots carry the five displayed H1 symbols directly: XAUUSD, GBPUSD, GBPAUD, GBPCAD and GBPJPY. All five calculate H3/H6/H9/H12/H14/H16 Monday-Friday; weekends are off and EURUSD is retired from H1 only. XAUUSD alone owns pattern/SW-BT entry-time detection for each block, and every row copies that XAUUSD entry hour. Once the entry hour is known, each row reads its own same-broker-day H(entry-1) candle from four closed M15 quarters to derive BUY/SELL, so entry H4 reads that row's H3 candle. GBPUSD therefore owns its final direction independently instead of synchronizing another row's side. The signal remains null until that H1 candle has fully closed. H16 stays entry/evidence only with no base direction, scheduled side or BUY/SELL. Telegram signal-slot mapping still ends at H14; broker-order scheduling is separate and unaffected. Legacy Thursday/Friday special propagation and CẦU presentation remain disabled. Legacy cloud scanner/backfill endpoints remain authenticated no-ops, so cTrader cannot overwrite rule-v86 state. Retained v85 pattern rows and retired EURUSD H1 rows are not relabeled as v86; a 90-day local source backfill rebuilds History after deployment.

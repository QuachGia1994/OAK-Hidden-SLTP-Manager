...
H1 rule v74 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating `H3/H6/H9/H12/H14/H16`.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route. Each live/backfill snapshot carries the previous available broker day for every signal-base source: GBPUSD, AUDUSD, USDCAD and USDJPY. Rule v74 keeps one shared pattern/evidence + entry-time driver for the three GBP crosses: GBPUSD M15. Their normal block signal bases remain independent: GBPAUD reads AUDUSD H(entry-1), GBPCAD reads USDCAD H(entry-1), and GBPJPY reads USDJPY H(entry-1), all from the previous available broker day; rising => BUY and falling => SELL with no extra GBP-cross inversion. Block eligibility is explicit Tuesday-Friday: GBPAUD uses H3/H6/H9/H12/H14/H16, GBPJPY uses H6/H9/H12/H14/H16, and GBPCAD uses H9/H12/H14/H16 like GBPUSD/EURUSD. Monday remains XAUUSD-only. GBPUSD/EURUSD keep their later-block GBPUSD pattern evidence and XAUUSD-side synchronization; their H9/H12/H14/H16 entry time comes directly from the XAUUSD entry for the same block. H16 entry time/pattern evidence is never changed by the H16 override. Instead, XAUUSD H3 determines one all-symbol H16 rule: if XAUUSD H3 entry is H5, each XAUUSD/FX H16 signal copies that same symbol's H14 signal and no CLOSE badge is shown; if XAUUSD H3 entry is H4, each H16 signal is the inverse of that same symbol's H14 signal and the H16 CLOSE badge is shown. CLOSE is presentation/advisory only and there is no automatic broker-close path. Legacy Thursday/Friday special propagation and CẦU presentation remain disabled. Legacy cloud scanner/backfill endpoints remain authenticated no-ops, so cTrader cannot overwrite rule-v74 state. History uses the same local source and can be seeded for up to 90 calendar days.

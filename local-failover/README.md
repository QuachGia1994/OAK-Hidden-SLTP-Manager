...
- Runtime EA identity: EA v1.14 heartbeats include a stable terminal-scoped `terminalId`. With `InpLocalProviderAccountId` blank, the EA derives `providerAccountId` from the currently logged-in `login`+`server`, detects account switches while it remains attached, and republishes the new identity. The controller reconciles that fresh heartbeat into the protected local-primary snapshot, preserves configured aliases such as `FXCE`, removes a stale duplicate alias instead of leaving ambiguous routing, and reapplies the user-only Windows ACL after the atomic config rewrite. `entry_prepare` and exact position-comment verification were introduced in EA v1.08; scheduled UI entry requires EA v1.13+ for same-direction TP rolling/reversal-gap evidence, while H1-calculated roll-only milestones require EA v1.14+; older EAs are refused (fail closed).
...
- Realtime scheduler: the controller loop wakes at the nearest scheduled `dueAt` (ms granularity, 50ms anti-busy-spin floor) instead of waiting a fixed tick, so a command like `buy XAUUSD 0.01 13h00 @fxce` dispatches within milliseconds of its due time — no minute cron (Cloudflare/GitHub/Vercel) and no Upstash polling participate in the timing path. With `scheduledEntryExecution: "mt5-ui"`, only a due `entry` uses the UI adapter; immediate entry and every close/modify/partial/positions action stay on the retained EA mailbox path. Wall-clock remains authoritative for `dueAt`; duplicate execution is prevented by the durable `scheduled -> executing` transition plus per-origin claim/result fences. The adapter first asks EA v1.13 for `entry_prepare`. When one same-direction position is still open, EA skips a duplicate entry and advances that position's TP by exactly 20.0 price units for XAU/GOLD or 50 pips for FX, then the controller sends a deduplicated `TP moved` Telegram result. Otherwise EA validates lot/protection/market data and projected post-net exposure before mutation, tracks every successful opposite close/pending delete, waits for broker-confirmed net settlement using the bounded `InpEntryNetSettleTimeoutMs`, rechecks exposure/tick after netting, then returns UI fields. If netting changed the account but the replacement entry cannot reach submit, the result is `REVERSAL_INCOMPLETE` with current BUY/SELL exposure and Telegram requires a manual broker check; automatic replay stays disabled. The adapter then converts machine-precision lots to MT5 manual-entry text, fills, commits and clicks the exact MT5 order controls. Correct-lot snapshot verification releases the next queued entry immediately. It never calls global mouse/keyboard APIs. Timing evidence is persisted per intent and surfaced in the Telegram execution reply.
...
- Trade-event reporting stays off the broker-mutation critical path. EA v1.09+ writes local FILE_COMMON event evidence for the first SL reach at/through break-even per position, broker `DEAL_REASON_SL` exits, filled pending orders and broker-confirmed partial-close deals that leave position volume open; EA v1.12 adds `reversal_incomplete` evidence when netting already changed exposure but the replacement entry does not safely complete. The controller validates the event against the configured MT5 account + fresh EA identity, sends Telegram, retains failed deliveries for retry, and remembers account-scoped delivered event IDs across restarts to suppress callback/file replays without suppressing the same broker deal number on another account. Successful timed entry/order, EA v1.13 scheduled TP-roll results, and timed close intents are separately queued for Telegram only after their broker/UI execution result is `executed` + `ok`; a Telegram outage never causes the broker mutation to run again.
...
- If a retained final result exists for the same origin/digest, it is reconciled without broker re-execution. A claim without a final result becomes `UNCERTAIN`; automatic replay is disabled. EA v1.12 also distinguishes the pre-submit reversal gap: when `entry_prepare` already changed opposite exposure and a later pre-submit step fails, it returns durable `REVERSAL_INCOMPLETE` evidence with closed-position/pending counts and current BUY/SELL exposure; Telegram raises a dedicated manual-check warning and the controller does not replay. After a UI submit, success is accepted only when the EA positions snapshot contains the exact `OAK:<ledger-prefix>` comment plus the expected symbol, side and lot; missing proof remains `UNCERTAIN`, never an automatic retry. This is a durable fail-closed fence, not a claim of absolute exactly-once broker execution.
...
H1 rule v97 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API for retained M15/H1 bars and never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the five active blocks `H3/H6/H9/H12/H14`; H16 remains removed as a block.

The local market-source set is `XAUUSD` + `GBPUSD` + `GBPAUD`. XAUUSD remains the only pattern/SW-BT detector and shared Entry-time owner; its internal GBPUSD M15 `entry - 15m` signal is retained only for internal XAU TP-roll behavior and is not published as a signal row. Public signals use the immediately previous XAU Entry time as the H1 reference: H3 uses the previous trading day's H14 Entry (for example H16 when that was the prior H14 Entry), while H6/H9/H12/H14 use the same-day H3/H6/H9/H12 Entry respectively. GBPUSD reads GBPAUD H1 at that previous Entry and KEEPS its raw direction; GBPAUD reads GBPUSD H1 at that previous Entry and INVERTS it. Live and backfill snapshots retain both current-day and previous-trading-day H1 context so H3 can resolve across weekends; missing previous Entry or exact H1 evidence fails closed instead of falling back to `block-1`. Each live v97 scan writes a current-day TP-milestone snapshot using the current block's shared XAUUSD Entry Time converted from IC Markets broker wall clock to an exact DST-aware epoch; the local-primary controller accepts only the matching v97 milestone contract and turns valid rows into deterministic EA v1.14 `tp_roll` intents. A roll-only intent can advance one same-direction TP by the existing XAU +20.0 / FX +50-pip step or no-op; it cannot open a trade or net opposite exposure. A Telegram scheduled entry for the same account/symbol/due time supersedes the automatic H1 roll so TP is never advanced twice at one milestone. Public schema 18 and cloud state v56 remain stable; stale v96 public rows are rejected, and a valid v97 public replica can reconstruct the hidden internal XAU entry-owner metadata plus both previous-entry cross-H1 rows for recovery.

...
- Local mutation intents use `L-<epoch>-<seq>` IDs. Timed entry/close intents auto-arm when saved; immediate mutations still require `/approve L-...`. Bare numeric cloud IDs are rejected. Multi-line messages remain atomic at the 10-line limit, and multiple MT5 accounts require explicit `@ACCOUNT` targeting.
...
The same Telegram bot accepts the core MT5 grammar: `/status`, `/profiles`, `/positions [@ACCOUNT]`, `/pending`, `/buy`, `/sell`, `/close`, `/closeall`, `/modify`, `/partial`, `/approve`, and `/del`. Scheduled `HH:MM` / `HHhMM` entry/close syntax auto-arms on creation and executes at the due time without `/approve`; immediate mutations keep the separate approval step, for example `/approve L-<epoch>-<seq>`.

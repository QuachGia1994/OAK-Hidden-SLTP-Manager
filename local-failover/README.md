...
H1 rule v95 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API for retained M15/H1 bars and never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the five active blocks `H3/H6/H9/H12/H14`; H16 is removed.

The local market-source set remains only `XAUUSD` + `GBPUSD`: XAUUSD owns pattern/SW-BT detection and the shared Entry time, while the final XAUUSD signal reads the exact same-broker-day GBPUSD M15 candle at `entry - 15m` (H3/H12/H14 invert; H6/H9 keep). Each live v95 scan also writes a local current-day TP-milestone snapshot: every populated public row uses that block's shared XAUUSD Entry Time, converted from IC Markets broker wall clock to an exact DST-aware epoch. The local-primary controller turns those milestones into deterministic EA v1.14 `tp_roll` intents for each fresh enabled MT5 account. A roll-only intent can advance one same-direction TP by the existing XAU +20.0 / FX +50-pip step or no-op; it cannot open a trade or net opposite exposure. A Telegram scheduled entry for the same account/symbol/due time supersedes the automatic H1 roll so TP is never advanced twice at one milestone. Rule v95 adds two derived public rows without scanning new symbols. GBPUSD H3 copies XAUUSD H3; for each later block, the immediately previous XAUUSD block decides the source (`entry-block = 1` -> current XAUUSD block, `entry-block = 2` -> previous XAUUSD block). GBPAUD H3/H6 invert XAUUSD on Monday/Tuesday/Thursday and keep Wednesday/Friday; H9/H12 invert Thursday/Friday and keep Monday/Tuesday/Wednesday; H14 is intentionally blank because no rule is defined. Missing required source evidence fails closed. Public schema 18 and cloud state v56 stay stable while signal rule v95 rejects stale v94 public rows; retained History is rebuilt from the 90-day local ICMarkets source after deployment.

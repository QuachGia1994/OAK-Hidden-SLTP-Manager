...
The V2 behavioral source of truth is `behavior-cases.json` plus `failover-v2.behavior.test.mjs` (38 numbered cases plus scheduled-entry routing coverage; 27–32 cover local-primary ownership, runtime identity, web sync, the cloud fence heartbeat and overdue expiry; 33–38 cover exact-dueAt realtime dispatch, controller restart before dueAt, Telegram outage/reconnect, web-sync outage non-blocking, deterministic multi-account @ACCOUNT routing and fence heartbeat renewal/expiry). `mt5-ui-entry-adapter.test.mjs` adds focused coverage for preparation, no-replay claims, post-submit uncertainty, routing isolation and the no-global-input contract. Offline verification covers cloud/local origin parity, atomic origin-claim races, retained results, fail-closed uncertainty, Telegram ownership handoff/recovery and installer dry-runs. Cloud-side fence gating is contract-checked in `dashboard/src/lib/telegram-cloud-route.test.ts`.

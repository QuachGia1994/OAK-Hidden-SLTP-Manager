...
H1 rule v89 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the six active blocks `H3/H6/H9/H12/H14/H16`.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route for exactly five sources: `XAUUSD`, `GBPUSD`, `AUDUSD`, `USDCAD`, `USDJPY`. XAUUSD alone owns pattern/SW-BT detection and therefore the shared Entry time. Once an entry hour is known, every table row reads the exact same-broker-day M15 candle at `entry - 2h15` from its own symbol; entry H11 therefore reads 08:45 on XAUUSD for the XAU row, 08:45 on AUDUSD for the AUD row, and so on. XAUUSD/GBPUSD/AUDUSD keep T=BUY and G=SELL, while USDCAD/USDJPY invert that base signal. H16 follows the same table rule. Telegram appointment synchronization remains capped separately at H14; legacy GBPAUD/GBPCAD/GBPJPY scheduled commands route their table annotation to AUDUSD/USDCAD/USDJPY respectively, without changing the broker order target or execution path. Stale v88 local-pattern rows are rejected rather than relabeled, so History must be rebuilt from the 90-day local M15 source after deployment.

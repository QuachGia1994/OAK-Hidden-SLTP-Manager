...
H1 rule v78 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the five active blocks `H3/H6/H9/H12/H14`.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route. Each live/backfill snapshot carries the previous available broker day for every signal-base source: GBPUSD, AUDUSD, USDCAD and USDJPY. Rule v78 keeps GBPUSD M15 as the shared pattern/evidence driver for the GBP crosses. Their normal signal bases remain independent: GBPAUD reads AUDUSD H(entry-1), GBPCAD reads USDCAD H(entry-1), and GBPJPY reads USDJPY H(entry-1), all from the previous available broker day; rising => BUY and falling => SELL with no extra GBP-cross inversion. Block eligibility Monday-Friday is now: GBPAUD H3/H6/H9/H12/H14, GBPUSD/EURUSD/GBPCAD H9/H12/H14, and GBPJPY H6/H9 only. GBPUSD and EURUSD H9/H12/H14 retain their GBPUSD-driven pattern/evidence window but copy the exact XAUUSD entry hour and already-final BUY/SELL for the same block, so Evidence FINAL SOURCE is XAUUSD with `SYNC XAUUSD`. H16 is retired: it is not calculated, published, synchronized to the H1 matrix, exported to PNG, or retained in rebuilt History. The Telegram broker-order scheduler is separate and continues executing accepted intents independently from this H1 display-slot retirement. Legacy Thursday/Friday special propagation and CẦU presentation remain disabled. Legacy cloud scanner/backfill endpoints remain authenticated no-ops, so cTrader cannot overwrite rule-v78 state. History uses the same local source and can be seeded for up to 90 calendar days.

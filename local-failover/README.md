...
H1 rule v63 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating `H3/H6/H9/H12/H14/H16`.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route. Each live/backfill snapshot also carries GBPUSD M15 bars from the previous available broker day, and EURUSD is now read as its own local ICMarkets M15 source for pattern/entry-time detection. After a pattern yields an entry hour, the server builds the previous broker day's GBPUSD H1 candle one hour before entry and maps bullish to BUY / bearish to SELL immediately. GBPAUD and GBPCAD H3/H6 invert that base signal so GBPCAD follows GBPAUD there. From H9/H12/H14/H16, GBPUSD and EURUSD take the final XAUUSD signal for the same block; Thursday flips GBPUSD once and Friday flips EURUSD once. If the first XAUUSD entry of the day is H5, the XAUUSD H16 final side is flipped and that already-final XAU side is what GBPUSD/EURUSD inherit before their weekday-specific flip. Monday remains XAUUSD-only with all FX rows blank. Legacy Thursday/Friday special propagation and CẦU presentation remain disabled. Legacy cloud scanner/backfill endpoints remain authenticated but are no-ops, so cTrader cannot overwrite rule-v63 state. History uses the same local source and can be seeded for up to 90 calendar days.

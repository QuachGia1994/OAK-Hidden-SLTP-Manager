...
A real `-Action Install` is a separate operator-authorized step. Its task definition runs as the current interactive Windows user, quotes the controller path, uses `MultipleInstances IgnoreNew`, keeps the logon trigger, and adds a one-minute repeating self-heal trigger so an externally terminated controller is relaunched without waiting for the next logon. Restart-on-failure is retained, and battery transitions do not stop or block the controller. To intentionally keep local control stopped, disable or uninstall the task rather than only stopping its current process. Restart/reload each MT5 terminal after compiling EA v1.13 so scheduled TP rolling, projected-exposure preflight, bounded net settlement and `REVERSAL_INCOMPLETE` evidence are active. Healthy operation remains `STANDBY`.

## Local ICMarkets H1 scanner

H1 rule v90 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the five active blocks `H3/H6/H9/H12/H14`; H16 is removed.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route for exactly five signal sources: `XAUUSD`, `GBPUSD`, `AUDUSD`, `USDCAD`, `USDJPY`. XAUUSD alone owns pattern/SW-BT detection and therefore the shared Entry time. Once an entry hour is known, every table row reads the exact same-broker-day M15 candle at `entry - 2h15` from its own symbol; XAUUSD/GBPUSD/AUDUSD keep T=BUY and G=SELL, while USDCAD/USDJPY invert that intrinsic base. Rule v90 then applies a final selector from XAUUSD H3 Entry: H3/H6/H9 use the previous available broker day's H3 Entry (`H4` keep, `H5` invert); H12/H14 use today's H3 Entry with the same selector. The local publisher therefore carries the previous available XAUUSD broker day as selector context only; all five signal bases remain same-day/own-symbol. Missing H3 selector evidence fails closed. Telegram appointment synchronization remains capped separately at H14; legacy GBPAUD/GBPCAD/GBPJPY scheduled commands still route table annotation to AUDUSD/USDCAD/USDJPY without changing the broker order target or execution path. Stale v89 local-pattern rows are rejected rather than relabeled, so History must be rebuilt from the 90-day local M15 source after deployment.

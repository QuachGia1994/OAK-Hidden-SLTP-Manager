...
H1 rule v84 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the six active blocks `H3/H6/H9/H12/H14/H16`.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route. Each live/backfill snapshot carries current-day AUDUSD, USDCAD and USDJPY scanner bars plus previous-broker-day GBPUSD bars needed by the H3-H14 final-signal base. Rule v84 gives each GBP cross its own scanner: GBPAUD reads AUDUSD M15, GBPCAD reads USDCAD M15 and GBPJPY reads USDJPY M15; all three now calculate the same H3/H6/H9/H12/H14/H16 weekday block set and each independently determines pattern family/SW-BT entry hour. On H3-H14 their final BUY/SELL base is previous-broker-day GBPUSD H(entry-2), so entry H8 reads GBPUSD H6; XAUUSD continues using GBPUSD at H(entry-2) on signal-bearing blocks. GBPUSD/EURUSD remain active H9/H12/H14/H16 and copy the exact XAUUSD entry hour + final BUY/SELL only on H9/H12/H14. H16 is deliberately entry-only for every row: the scanner publishes pattern/evidence and entry hour, but base direction, scheduled side and BUY/SELL remain blank/null and Evidence labels `ENTRY ONLY`. Telegram signal-slot mapping still ends at H14; the broker-order scheduler is separate and continues executing accepted intents independently from the H1 display matrix. Legacy Thursday/Friday special propagation and CẦU presentation remain disabled. Legacy cloud scanner/backfill endpoints remain authenticated no-ops, so cTrader cannot overwrite rule-v84 state. History uses the same local source and can be seeded for up to 90 calendar days.

...
H1 rule v67 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating `H3/H6/H9/H12/H14/H16`.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route. Each live/backfill snapshot also carries GBPUSD M15 bars from the previous available broker day. Pattern/evidence and entry timing are intentionally decoupled where required: GBPUSD H9/H12/H14/H16 keeps its own GBPUSD local-M15 pattern-derived entry hour; EURUSD H9/H12/H14/H16 takes the same entry hour/pattern source as GBPUSD; GBPCAD and GBPJPY H3/H6 take GBPAUD entry timing; and GBPCAD/GBPJPY H9/H12/H14/H16 take GBPUSD entry timing. GBPCAD still validates H9+ pattern/evidence through GBPJPY/USDJPY. Final-side mapping is explicit: GBPCAD + GBPJPY H3/H6 copy GBPAUD's final signal, H9/H12 copy XAUUSD's final signal, and H14/H16 keep the GBPUSD-derived entry hour/evidence but publish no BUY/SELL signal. GBPUSD and EURUSD keep their existing XAUUSD-side synchronization, with Thursday flipping GBPUSD and Friday flipping EURUSD. If the first XAUUSD entry of the day is H5, the old H16 reversal is not applied: H16 becomes a manual CLOSE-only advisory block and every H16 BUY/SELL signal is suppressed to null while entry/pattern evidence remains available. This CLOSE badge has no broker mutation path and never auto-closes a position; the user decides whether to close. GBPCAD/GBPJPY H16 remain signal-blank by their own rule as well. Monday remains XAUUSD-only with all FX rows blank. GBPAUD H3/H6 and GBPUSD H9/H12/H14/H16 are highlighted in the web table as entry-reference cells. Legacy Thursday/Friday special propagation and CẦU presentation remain disabled. Legacy cloud scanner/backfill endpoints remain authenticated but are no-ops, so cTrader cannot overwrite rule-v67 state. History uses the same local source and can be seeded for up to 90 calendar days.

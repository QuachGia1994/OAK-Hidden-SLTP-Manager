...
H1 rule v99 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API for retained M15/H1 bars and never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the four active blocks `H3/H6/H9/H12`; H14 is no longer a block, though H12 can still resolve to entry H14.

The local market-source set remains `XAUUSD` + `GBPUSD` + `GBPAUD`. XAUUSD is the only pattern/SW-BT detector and shared Entry-time owner; only H3 determines the entry delta, which cascades across H6/H9/H12. H3 H4 yields H6/H7, H9/H10, H12/H13; H3 H5 yields H6/H8, H9/H11, H12/H14. The public feed/table contains only GBPAUD, which reads same-day GBPAUD H1 at `entryHour - 1`; raw `T` is BUY and raw `G` is SELL. There is no delta inversion or previous-block/previous-day selector, and missing exact GBPAUD H1 evidence fails closed. GBPUSD remains internal for XAUUSD private M15 TP-roll handling. Each live v99 scan writes current-day TP milestones using the shared XAUUSD Entry time, and the local-primary controller accepts only the matching v99 milestone contract. Public schema 18 and cloud state v56 remain stable; stale prior-rule rows are rejected and the valid v99 public replica reconstructs the hidden XAU Entry owner plus the single GBPAUD signal row.

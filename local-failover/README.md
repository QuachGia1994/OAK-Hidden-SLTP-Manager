...
H1 rule v87 is independent of the trading EA and reads market data only from the logged-in ICMarkets MT5 terminal. `mt5-h1-market-reader.py` uses the MetaTrader5 Python API with `copy_rates_from_pos(..., TIMEFRAME_M15, ...)`; it never calls `order_send` or any position mutation API. On this MT5 API path the rate epoch fields expose terminal/server-wall components, so the reader decodes them directly and deliberately does not reapply the cTrader UTC→ICMarkets `UTC+2/UTC+3` conversion before evaluating the six active blocks `H3/H6/H9/H12/H14/H16`.

The publisher posts T/G + OHLC M15 evidence to the authenticated `/api/h1-scanner/local-market` route. Rule v87 reads eight market sources: the five displayed rows plus AUDUSD/USDCAD/USDJPY signal bases. XAUUSD alone owns pattern/SW-BT entry-time detection for every row. On H3-H14, XAUUSD reads previous available broker-day GBPUSD H(entry-1), GBPAUD reads previous-day AUDUSD, GBPCAD previous-day USDCAD and GBPJPY previous-day USDJPY; Monday therefore resolves Friday. GBPUSD keeps its current same-day own GBPUSD H(entry-1) rule and waits until that H1 candle closes. H16 stays entry/evidence only. Live/history snapshots carry both current-day and previous available broker-day bars so the server can resolve these mapped bases. Telegram signal-slot mapping and broker-order scheduling remain separate and unaffected. Stale v86 mapped rows are rejected rather than relabeled, so History needs a 90-day local source rebuild after deployment.

...
4. **Ghost Operator (Stealth Mode)**: Giả lập thao tác tay khi Algo bị chặn hoặc để che giấu Robot 100%. Có thể bật thủ công tại Sidebar/Dashboard.
5. **Full Localization**: Hỗ trợ chuyển đổi ngôn ngữ VN/EN cho toàn bộ giao diện, tin tức và lịch trình nhắc nhở.
6. **Chốt lời từng phần (Auto Partial Close)**: Tự động cắt bớt volume khi lệnh đạt mức lợi nhuận mong muốn. Hỗ trợ cú pháp tiếng Việt linh hoạt ("lụm", "bỏ túi",...).
...
## ⚠️ Lưu ý quan trọng
- **Algo Trading**: Phải bật nút "Allow automated trading" trên MT5 (nếu dùng chế độ API mặc định).
- **Ghost Mode**: Nếu sàn chặn Algo, hãy bật nút **"BẬT GHOST MODE"** màu xanh để Robot chuyển sang giả lập thao tác tay.
